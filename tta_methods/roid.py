    ...
    def loss_calculation(self, x1, x2):
        ...
        if self.use_weighting:
            with torch.no_grad():
                ...
                B, C, W, H = outputs[-1].shape
                if self.class_probs_ema.dim() == 1:
                    self.class_probs_ema = self.class_probs_ema.unsqueeze(0).unsqueeze(2).unsqueeze(2).repeat(B, 1, H, W)
                ...
                weights_cert = - self.softmax_entropy(logits=outputs[-1])
                ...
                self.class_probs_ema = update_model_probs(x_ema=self.class_probs_ema, x=outputs[-1].softmax(1).mean(0), momentum=self.momentum_probs)
        ...
        #     loss += (self.symmetric_cross_entropy(x=outputs_aug[-1], x_ema=outputs[-1][~mask]) * weights[~mask]).sum() / self.batch_size
