...
class CoTTA(TTAMethod):
    ...
    def loss_calculation(self, x1, x2):
        ...
        anchor_prob = torch.nn.functional.softmax(self.model_anchor(x1, x2)[-1], dim=1).max(1)[0]

        # Augmentation-averaged Prediction
        ema_outputs = []
        outputs_ema = self.model_ema(x1, x2)
        if anchor_prob.mean() < self.ap:
            for _ in range(self.n_augmentations):
                outputs_ = self.model_ema(self.transform(x1), self.transform(x2)).detach()
                ema_outputs.append(outputs_[-1])

            # Threshold choice discussed in supplementary
            outputs_ema[-1] = torch.stack(ema_outputs).mean(0)


        loss = self.softmax_entropy(outputs[-1], outputs_ema[-1]).mean()
    ...
    def forward_and_adapt(self, x1, x2):
        if self.mixed_precision and self.device == "cuda":
            with torch.cuda.amp.autocast():
                outputs_ema, loss = self.loss_calculation(x1, x2)
