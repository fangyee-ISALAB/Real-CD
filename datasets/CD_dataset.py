...
class ImageDataset(data.Dataset):
    """VOCdataloder"""
    def __init__(self, root_dir_A, root_dir_B, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        self.list_path = os.path.join(self.root_dir_A, LIST_FOLDER_NAME, self.split+'.txt')
    ...
    def __getitem__(self, index):
        ...
        A_path = get_img_path(self.root_dir_A, self.img_name_list[index % self.A_size])
        B_path = get_img_post_path(self.root_dir_B, self.img_name_list[index % self.A_size])
...
class CDDataset(ImageDataset):
    ...
    def __init__(self, root_dir_A, root_dir_B, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(CDDataset, self).__init__(root_dir_A, root_dir_B, img_size=img_size, split=split, is_train=is_train,
                                        to_tensor=to_tensor)
    ...
    def __getitem__(self, index):
        ...
        img = np.asarray(Image.open(A_path).convert('RGB'))
        img_B = np.asarray(Image.open(B_path).convert('RGB'))
        L_path = get_label_path(self.root_dir_A, self.img_name_list[index % self.A_size])
